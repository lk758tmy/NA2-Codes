...
float Richardson(float *m,float *x,float *c,int n,int step,float *r){
    float tau,residual=0;
    for(int k=1;k<=step;k++,cnt++){
        tau=1/(cos((2*k-1.0)*M_PI/2/step)*L1+L2); //Can optimize?
        if(residual<1e-8){
...
int main(){
	...
	for(int i=2;i<=t;i++){
        cnt=0;
        for(int j=0;j<n2;j++) *(x+j)=*(r+j)=0;
        printf("%d\t%.8f\t",i,Richardson(m,x,c,n2,i,r));
        for(int j=0;j<n2;j++) *(x+j)-=1;
        printf("%.8f\n",cblas_snrm2(n,x,1));
	}
