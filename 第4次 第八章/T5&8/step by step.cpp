...
int Cl_Jacobi(double *m,int n,double *eigenReal){
	double *eigenNum=(double *)malloc(n*sizeof(double));
	int maxI,maxJ,cnt=0,cycLen=n*(n-1)/2; double maxValue,c,s,t,kse,errf;
	...
	do{
		...
		if((cnt*5)%cycLen==0){
			...
			cblas_daxpy(n,-1,eigenReal,1,eigenNum,1);
			errf=cblas_dnrm2(n*n,m,1); errf-=cblas_dnrm2(n,m,n+1);
			printf("%.1f,%.3e,%.3e\n",double(cnt)/cycLen,cblas_dnrm2(n,eigenNum,1),errf);
		}
	}while(1);
	...
	printf("%.0f,",ceil(double(cnt)/cycLen));
	return cnt;
}
int Cy_Jacobi(double *m,int n,double *eigenReal){
	double *eigenNum=(double *)malloc(n*sizeof(double));
	int cnt=0,cntCycle=0; double c,s,t,kse,errf; bool flag;
...
int T_Jacobi(double *m,int n,double delta,double *eigenReal){
	double *eigenNum=(double *)malloc(n*sizeof(double));
	int cnt=0,cntCycle=0; double c,s,t,kse,th,errf; bool flag;
	th=cblas_dnrm2(n*n,m,1); th-=cblas_dnrm2(n,m,n+1);
	
	cblas_dcopy(n,m,n+1,eigenNum,1);
	sort(eigenNum,eigenNum+n);
	cblas_daxpy(n,-1,eigenReal,1,eigenNum,1);
	printf("0,%.3e,%.3e\n",cblas_dnrm2(n,eigenNum,1),th);
...
int main(){
	...
	for(int q=0;q<3;q++){
		...
		case 0:
			printf("Classic,%d\n",n);
			Cl_Jacobi(m,n,eigenReal); break;
		case 1:
			printf("Cyclic,%d\n",n);
			Cy_Jacobi(m,n,eigenReal); break;
		case 2: 
			printf("Threshold,%d\n",n);
