...
void cholesky_llt(float *m,float *x,float *c,int n){
	...
	for(int i=0;i<n;i++){
        *(m+i*n+i)=sqrt(*(m+i*n+i)-cblas_sdot(i,m+i*n,1,m+i*n,1));
        tmp=1/(*(m+i*n+i));
        for(int j=i+1;j<n;j++){
            *(m+j*n+i)-=cblas_sdot(i,m+i*n,1,m+j*n,1);
            *(m+j*n+i)*=tmp;
        }
        for(int j=0;j<i;j++) *(m+j*n+i)=0;
...
int auto_main(int n) {
	...
	//matrix_print(m,n2);  vector_print(x,n2);
	e=clock();
